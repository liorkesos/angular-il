<?php // Modified for WPH - Localized error messages
/**
 * WordPress DB Class
 *
...
class wpdb {
	...
	function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
		...
		if (!$this->dbh) {
		$this->bail("
			<h1>בעיה בחיבור לבסיס הנתונים</h1>
			<p>זה אומר ששם המשתמש והסיסמה בקובץ <code>wp-config.php</code> שלך אינם נכונים, או שאי אפשר להתחבר לשרת בכתובת <code>$dbhost</code>. ייתכן שהשרת לא פעיל כרגע.</p>
			<ul>
				<li>כדאי לוודא ששם המשתמש והסיסמה נכונים.</li>
				<li>כדאי לוודא שכתובת שרת בסיס הנתונים נכונה.</li>
				<li>כדאי לוודא שהשרת אכן פעיל וזמין כרגע.</li>
			</ul>
			<p>אם כל זה לא אומר לך כלום, כדאי לך לפנות לתמיכה של ספקי האחסון שלך בטלפון או בדואל. לחלופין, אפשר תמיד לפנות <a href='http://groups.google.com/group/wpheb/topics'>לקבוצת הדיון של וורדפרס בעברית</a>.</p>
			");  // WPH - Localized error message
			return;
		}
	...
	function select($db) {
		if (!@mysql_select_db($db, $this->dbh)) {
			$this->ready = false;
						$this->bail("
			<h1>אי אפשר לבחור את בסיס הנתונים</h1>
			<p>אפשר להתחבר לשרת (כלומר, שם המשתמש והסיסמה נכונים), אבל אי אפשר לבחור את בסיס הנתונים <code>$db</code>.</p>
			<ul>
			<li>כדאי לוודא שבסיס הנתונים אכן קיים ושאין שגיאות בשם שלו.</li>
			<li>במערכות מסוימות, שם בסיס הנתונים מתחיל תמיד בשם המשתמש שלך - למשל, username_wordpress. שווה לנסות.</li>
			</ul>
			<p>אם כל זה לא אומר לך כלום, כדאי לך לפנות לתמיכה של ספקי האחסון שלך בטלפון או בדואל. לחלופין, אפשר תמיד לפנות <a href='http://groups.google.com/group/wpheb/topics'>לקבוצת הדיון של וורדפרס בעברית</a>.</p>
			"); // WPH - Localized error message
			return;
